...
#if defined(NDEBUG)
# error "Guangcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Guangcoin Signed Message:\n";
...
//TODOGUANGfix Subsidy to 2/(x**2 + 2) decay
// - with 10 coins per block start, we end up with ~2.3 million coins + the tail emission constant
// so we make it 20 Coins by default, which means we arrive at around 9.9 million coins after 60 years [- PoB coins], 5.3 million after 5 years
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    // nSubsidyHalvingInterval is now just a "timebase"
    double halvings = (double) nHeight / consensusParams.nSubsidyHalvingInterval;
    // Force block reward to zero when right shift is undefined.
    //if (halvings >= 64)
    //    return 1;  //tail emission
    
    //expanded multiplication here to make it clear that the 2 is from the numerator of our decay function
    CAmount nSubsidy = (1 + 2 * 20) * COIN; //smaller subsidy to start with, wrt btc, ltc
    // Subsidy is cut in half every halvinginterval blocks which will occur approximately every xxx.
    nSubsidy = nSubsidy / (halvings * halvings + 2);
    return nSubsidy + COIN/2; //tail emission
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
	...
	// TODOGUANG ": modify block reward validation to match our payment schedule - also need to validate the tx burn component
    if (block.vtx[0]->GetValueOut() > 0.95 * blockReward)
