                  ...
                  picnic_privatekey_t* sk)
{
    ...
    // TODOGUANG - we need a *deterministic* plaintext for our pubKey, so it is regeneratable from the same privKey
    // plaintext is 32bytes long and so we use the following - we need to save the 32 bit plaintext with the privKey if we export it
